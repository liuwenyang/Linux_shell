...
function startcleardockerlogtask() {
  ...
  echo -e "[Unit]\nDescription=Periodic cleaning zc docker logs\n\n[Service]\nExecStart=/bin/bash "$(pwd)"/clearzcfiles_train.sh -o" >/etc/systemd/system/clearzcdockerlogs.service
