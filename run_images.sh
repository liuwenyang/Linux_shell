...
if [ -z "$(docker ps | awk 'BEGIN{FS=" "} $3 ~/dataview/ {print $1}')" ]; then
    docker run --net=host -v /home/storage:/home/storage:rw -v /etc/timezone:/etc/timezone:ro -v /etc/localtime:/etc/localtime:ro --restart always --name=dataview -itd dataview && echo "-----dataview容器加载完成--------"
fi
